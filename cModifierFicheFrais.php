 ...
 */
  ...
  if ( !$existeFicheFrais ) {
      ajouterFicheFrais($idConnexion, $mois, $visiteursaisi);
  }
  ...
  if ($etape == "validerSaisie") { 
      ...
      if (!$ok) {
          ...
          modifierEltsForfait($idConnexion, $mois, $visiteursaisi,$tabQteEltsForfait);
      }
  }                                                       
  ...
  elseif ($etape == "validerAjoutLigneHF") {
      ...
      if ( nbErreurs($tabErreurs) == 0 ) {
          // la nouvelle ligne doit être ajoutée dans la base de données
          ajouterLigneHF($idConnexion, $mois, $visiteursaisi, $dateHF, $libelleHF, $montantHF);
      }
  }
...
?>
  <!-- Division principale -->
  <div id="contenu">
      <h2>Renseigner la fiche de frais du mois de <?php echo obtenirLibelleMois(intval(substr($mois,4,2))) . " " . substr($mois,0,4); ?></h2>
<?php
...
<?php          
          ...
          ?>
              <tr>
                ...
                <td>
				<form action="<?php echo URL.'index.php?action=modifier&amp;etape=validerSuppressionLigneHF&amp;idLigneHF='.$lgEltHorsForfait["id"]; ?>" method="post">
				...
				<input id="supprimer" type="submit" value="Supprimer" onclick="return confirm('Voulez-vous vraiment supprimer cette ligne de frais hors forfait ?')" />
				</form>
		        </td>
              </tr>
          <?php
